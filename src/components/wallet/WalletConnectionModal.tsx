...
import { Wallet, Loader2, ExternalLink, Check, Smartphone } from "lucide-react";
...
export default function WalletConnectionModal({ trigger }: WalletConnectionModalProps) {
  ...
  const walletOptions: WalletOption[] = [
    {
      ...
      mobileDescription: isMobile && !isInWalletBrowser ? 'Tap to open Phantom app' : undefined,
      popular: true,
    ...
    },
    {
      ...
      mobileDescription: isMobile && !isInWalletBrowser ? 'Tap to open Solflare app' : undefined,
    ...
    },
    {
      ...
      mobileDescription: isMobile && !isInWalletBrowser ? 'Tap to open Backpack app' : undefined,
  ...
  // Wallet content shared between Dialog and Drawer
  const walletContent = (
    <>
      {wallet.isConnected ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  <code className="text-xs bg-secondary/50 px-2 py-0.5 rounded break-all max-w-[120px] sm:max-w-none">
          ...
          {isMobile && !isInWalletBrowser && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
              ...
              <p className="text-xs text-muted-foreground">
                Tap a wallet to open its mobile app. This page will load inside the app.
              </p>
            </div>
          )}
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {walletOpt.mobileDescription || walletOpt.description}
                  </p>
                </div>
    ...
    >
      {wallet.isConnected ? (
        <>
          ...
          <span className="hidden xs:inline">{wallet.address?.slice(0, 4)}...{wallet.address?.slice(-4)}</span>
          <Wallet className="w-4 h-4 xs:hidden" />
        </>
      ) : (
        <>
  ...
  // Use Drawer on mobile for better UX, Dialog on desktop
  if (isMobile) {
    return (
      <Drawer open={isOpen} onOpenChange={setOpen}>
        ...
        <DrawerContent className="border-border/50 bg-card/95 backdrop-blur-xl">
          ...
          {walletContent}
          <div className="h-6" /> {/* Bottom safe area */}
        </DrawerContent>
      </Drawer>
    );
  }
